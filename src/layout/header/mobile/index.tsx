...
const HeaderMobile = () => {
    ...
    const handleOpenMenu = (type) => {
        ...
        document.querySelectorAll(`.sub-menu.open`)?.forEach(e=>{
    ...
    return (
        <div ref={clickRef} className={"w-full"}>
            <div className="mobile">
                <CgMenuRight size={26} color={"#5C677D"} className={"cursor-pointer"}
                             onClick={() => handleOpenMenu(".modal-menu-mobile")}/>
                <Logo/>
                <RiSearch2Line size={26} color={"#424750"} className="cursor-pointer"
                               onClick={() => document.querySelector(".header__top-search-mobile").classList.add("active")}/>
            </div>
            <hr className="w-[91%] block lg:hidden mx-auto  mt-[50px] border-solid border-[#eee]"/>
