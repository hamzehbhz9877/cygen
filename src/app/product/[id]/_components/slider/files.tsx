            ...
            <ul className="main_gallery_product">
                ...
                {/*    product.VideoModels?.length > 0 ?*/}
                        <li data-remodal-target="modalvidoe" className="show_modal_gallery video"
                            onClick={() => openModal(<FilesModal data={product}  type={"video"} index={product.PictureModels.length} close={closeModal}/>,
                                {className:"!w-full !rounded-none !h-auto !m-0 !bg-black"})}
                        >
...

                {
                    product.PictureModels?.map((pic, index) => {
                        if (index <= 3)
                            return (
                                <li key={pic.Id} onClick={() => openModal(<FilesModal data={product}  type={"image"} index={index} close={closeModal}/>,{className:"!w-full !rounded-none !h-auto !m-0 !bg-black"})}>
                                    <Image loading="lazy" width="150" height="150"
                                           className="attachment-thumbnail size-thumbnail"
                ...
                {/*{product.PictureModels?.length > 4 ?*/}
                    <li className="show_modal_gallery" onClick={() => openModal(<FilesModal data={product} type={"image"} index={0} close={closeModal}/>,{className:"!w-full !rounded-none !h-auto !m-0 !bg-black"})}>
                        <Image loading="lazy" width="150" height="150"
                               className="attachment-thumbnail size-thumbnail"
                               alt={product.DefaultPictureModel.AlternateText}
                               title={product.DefaultPictureModel.Title}
